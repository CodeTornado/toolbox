	...
	methods: {
		submitForm(formName) {
			this.$refs[formName].validate((valid) => {
				if (valid) {
					var word = this.submitData.word;
					// alert('submit!' + this.submitData.word);
					var salt_uuid = getUUID();
					...
					var paramObj = {
						...
						'from': 'auto',
						'to': 'auto',
						'appKey': appId, // 这里是按照文档写的 没问题
						'salt': salt_uuid,
						'sign': SHA256(appId + input + salt_uuid + curtime + appkey),
						'signType': 'v3',
						'curtime': curtime,
						'ext': 'mp3'
					};
					console.log(JSON.stringify(paramObj));//参数正确可以返回数据
					
					//TODO: 未处理js 跨域问题! 真让人头大!
